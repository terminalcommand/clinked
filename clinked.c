...
struct node *processsublist(size_t startindex, size_t length, char **list) {
	struct node *head = makenode(NULL, NULL, list[startindex]);
	struct node *cursor = head;
	
	for (int i = startindex+1; i < length; i++) {
		struct node *new = makenode(cursor, NULL, list[i]);
		cursor->next = new;
		if (i == startindex) { head = cursor; }; // if running for the first time
		cursor = new;
	}
	return head;
}	
